...
Blockly.JavaScript.forBlock["tween_block"] = function (block, generator) {
  const easingType = block.getFieldValue("EASING_TYPE");
  const easingMode = block.getFieldValue("EASING_MODE");
  const from =
    generator.valueToCode(block, "FROM", Blockly.JavaScript.ORDER_ATOMIC) ||
    "0";
  const to =
    generator.valueToCode(block, "TO", Blockly.JavaScript.ORDER_ATOMIC) || "0";
  const duration =
...
Blockly.JavaScript.forBlock["tween_block_value"] = () => [
  "tweenValue",
...
Blockly.JavaScript.forBlock["tween_sprite_property"] = function (
  block,
  generator
) {
